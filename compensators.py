...
def bode(expression, n = 10):
	freqs = signal.findfreqs(e2sys(expression).num, e2sys(expression).den, n)
...
def lagCompensate(expression, target):
	...
	print phase
	print phaseTargetLocation, phaseTarget, phase[phaseTargetLocation]
	tau = 10/w[phaseTargetLocation]
	for alpha in numpy.logspace(-2, 5, ct): 
		# equation for basic lag compensator
		G_c = (tau*s+1)/(alpha*tau*s+1)
		if abs(phaseMargin(expression*G_c)['p_m']-target) < 0.1:
			break
